<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Arches and Avenues | Event Planning</title>
    <link rel="stylesheet" href="style.css">
</head>
<body>
<header>
    <h1>Arches and Avenues</h1>
    <nav>
        <ul>
            <li><a href="index.html">Home</a></li>
            <li><a href="offerings.html">Offerings</a></li>
            <li><a href="portfolio.html">About Us</a></li>
            <li><a href="contact.html">Contact Us</a></li>
        </ul>
    </nav>
</header>

<section class="hero">
    <div class="hero-overlay">
        <img src="download.jpg" alt="Wedding Event" class="hero-img">
        <div class="hero-text">
            <h2>Creating Unforgettable Events</h2>
            <p>Weddings • Parties • Corporate Events • Special Occasions</p>
            <p class="tagline">Turning special moments into lifelong memories</p>
            <a href="contact.html" class="btn">Book a Consultation</a>
        </div>
    </div>
</section>

<section id="services" class="services">
    <h2>Our Services</h2>
    <div class="service-grid">
        <div class="service-card">
            <span class="emoji">💍</span>
            <h3>Wedding Planning</h3>
            <p>Full‑service planning to make your special day flawless.</p>
        </div>
        <div class="service-card">
            <span class="emoji">💼</span>
            <h3>Corporate Events</h3>
            <p>Professional event coordination for businesses and brands.</p>
        </div>
        <div class="service-card">
            <span class="emoji">✨</span>
            <h3>Private Parties</h3>
            <p>Birthdays, anniversaries, and celebrations of all kinds.</p>
        </div>
    </div>
</section>

<footer>
    <p>© 2026 Arches and Avenues.</p>
</footer>

</body>
</html>
